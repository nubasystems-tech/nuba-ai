...
def _engine_for(lang: str):
    """قياس (2026-09-03 16:11-16:14، دورة كاملة بسجل): nova-3 سليم اليوم
    (en conf=0.998, fr conf=0.999 خارج uvicorn وضمن محاكاة 3 محركات متوازية).
    الكارثة السابقة: تحويل en/fr→Cohere جعل النص الصحيح يصل بثقة 0.00
    فيخسر أمام صدى Cohere ar (وزن محايد 0.85) — 4/4 جمل باتجاه معكوس.
    القرار: العربية → Cohere (مثبت 100%)، وكل ما عداها → nova-3 (ثقة حقيقية
    تحسم التحكيم ضد أي صدى). لو انقطع Deepgram مستقبلاً: يظهر في السجل
    «فشل فتح <lang>» — عندئذ فقط التحويل المؤقت إلى Cohere."""
    if lang == "ar":
        return "Cohere", ""
...
async def duo_lang_worker(user_ws: WebSocket, lang_a: str, lang_b: str, tts_on: bool = True):
    ...
    _reborn = {}             # lang → عدد مرات إعادة الفتح بالحارس
        ...
        while not finished:
            ...
            for l in list(upstreams.keys()):
                if l not in engine_got:
                    # 🎯 fallback الجلسة (قياس 16:14-16:20): nova-3 صامت من جهة
                    # Telnyx كلياً (حتى خارج uvicorn بنفس البايتات) بينما Cohere
                    # يعمل — بعد إعادة فتح ثالثة فاشلة نحوّل اللغة إلى Cohere
                    # نهائياً لهذه الجلسة (التحكيم يعوّل على الفلتر اللغوي).
                    ...
                    break   # واحد كل 2s (لا نغرق الافتتاحات)
    ...
    async def _crown(gid: str):
        ...
        if not valid:
            ...
            return
        # 🎯 قاعدة الكرسي الأجنبي (قياس 16:12-16:14): صدى Cohere ar للكلام
        # الأجنبي نص عربي صافٍ (يجتاز الفلتر) لكنه إعادة صياغة عربية للمعنى —
        # إن وصل في نفس النافذة متنافس عبر محرك لغة أجنبية (en/fr) مطابق
        # للغته، فالكلام كان أجنبياً قطعاً → المتنافس ar صدى: نحذفه.
        foreign_valid = [it for it in valid if it[0] != "ar"]
        if foreign_valid:
            valid = foreign_valid
        def _fair(it):
            lang, text, conf = it
...
async def multi_lang_worker(user_ws: WebSocket, tts_lang: str, fan_langs=None):
    ...
    async def _engine_guard():
        ...
        while not finished:
            ...
            for l in list(upstreams.keys()):
                if l not in engine_got:
                    # 🎯 fallback الجلسة (قياس 16:14-16:20): nova-3 صامت من جهة
                    # Telnyx — بعد أول إعادة فتح فاشلة نحوّل اللغة إلى Cohere.
                    force = "Cohere" if _reborn.get(l, 0) >= 1 else None
                    _reborn[l] = _reborn.get(l, 0) + 1
    ...
    async def _crown(gid: str):
        ...
        # 🎯 قاعدة الكرسي الأجنبي (نفس duo — قياس 16:11-16:20): صدى ar الصافي
        # للكلام الأجنبي يُحذف إن وُجد متنافس أجنبي مطابق للغته في النافذة.
        foreign_valid = [it for it in valid if it[0] != "ar"]
        if foreign_valid:
            valid = foreign_valid
        def _fair(it):
